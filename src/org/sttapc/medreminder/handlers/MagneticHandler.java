...
public class MagneticHandler {
	...
	public MagneticHandler(InterfaceKitPhidget interfaceKitPhidget,
			Configurator configurator) throws PhidgetException {

		this.interfaceKitPhidget = interfaceKitPhidget;
		// interfaceKitPhidget.setOutputState(configurator.getOutputOne(),
		// false);
		// interfaceKitPhidget.setOutputState(configurator.getOutputTwo(),
		// false);
		//
	 ...
	 * This method is responsible for handling to a corresponding state TODO:
	 * This method should be called after validation of correct user interaction
	 * example: If user wants to refill his case. If user accidentially opens it
	 * without taking the medication.
	 * 
	 */
	private void HandleState() {
		Date currentTime = new Date();
		org.sttapc.medreminder.context.State currentState = StateProvider
				.getInstance().getState();
		System.out.println("MagneticHandler HandleState - State = "
				+ currentState);
		switch (currentState) {
		/*
		 * If this State is triggered, it means that user has opened the case
		 * within the ACTIVE state. We assume the user is taking the medicine it
		 * changes state to IDLE Log info to a xml file.
		 */
		case ACTIVE:
			StateProvider.getInstance().setState(
					org.sttapc.medreminder.context.State.IDLE);
			try {
				logning.LogForMagneticHandler(currentTime, State.ACTIVE,
						adherence.CalculatePoints(currentTime));
			...
			break;
		/*
		 * If Case is opened in IDLE state, it should not do nothing. Maybe Log
		 * it ?
		 */
		case IDLE:
			try {
				System.out.println("MagneticHandler:" + currentTime
						+ State.IDLE + 9999);
		...
		case WARNING:
			StateProvider.getInstance().setState(
					org.sttapc.medreminder.context.State.IDLE);
			try {
				logning.LogForMagneticHandler(currentTime, State.WARNING,
						adherence.CalculatePoints(currentTime));
			} catch (TransformerException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	/**
	 * Sets whether the case is closed or opened. Also sets the date for when
	 ...
	 * write into xml file with validation Should only log for when user has
