...
public class Adherence {
	...
	public Integer CalculatePoints(Date currentDate) {
		...
		lowerIdealDate.setHours(schedule.getIdealDate().getHours()
				- hourWithInRange);
		upperIdealDate.setHours(schedule.getIdealDate().getHours()
				+ hourWithInRange);

		if (upperIdealDate.before(currentDate)
				&& lowerIdealDate.after(currentDate)) {
			return 3;
		}
		...
		if ((schedule.getIdealDate().getHours() + hourRangeOne < currentDate
				.getHours())) {
			return 2;
		}
